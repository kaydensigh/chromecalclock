...
var DatePicker = new Class({
	...
	options: {
		...
		debug: false,
		toggleElements: null,
		persistent: false, // whether this picker is always shown
	...
	renderMonth: function() {
		...
		for (d = this.options.startDay; d < (this.options.startDay + 7); d++) {
			new Element('div', { 'class': 'title day day' + (d % 7) }).set('text', this.options.daysShort[(d % 7)]).inject(titles);
		}
	...
	renderYear: function() {
		...
		for (i = 0; i <= 11; i++) {
			...
			.set('text', this.options.monthsShort ? this.options.monthsShort[i] : this.options.months[i]).inject(container);
	...
	format: function(t, format) {
		...
		for (var i = 0; i < format.length; i++) {
			switch(format.charAt(i)) {
				...
				case 'n': f += (m + 1); break;
				case 'M': f += this.options.monthsShort[m]; break;
				case 'F': f += this.options.months[m]; break;
				case 'd': f += this.leadZero(t.getDate()); break;
				case 'j': f += t.getDate(); break;
				case 'D': f += this.options.daysShort[t.getDay()]; break;
	...
	unformat: function(t, format) {
		...
		for (var i = 0; i < format.length; i++) {
			...
			switch(c) {
				...
				case 'M': r = '['+this.options.monthsShort.join('|')+']'; break;
				case 'F': r = '[A-Za-z]+'; break;
				case 'd': r = '0[1-9]|[12][0-9]|3[01]'; break;
				case 'j': r = '[1-9]|[12][0-9]|3[01]'; break;
				case 'D': r = '['+this.options.daysShort.join('|')+']'; break;
		...
		for (c in a) {
			...
			switch(c) {
				...
				case 'M': v = this.options.monthsShort.filter(function(item, index) { return item == v }.bind(this))[0];
